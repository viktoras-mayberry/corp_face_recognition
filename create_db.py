...
def create_database():
    ...
    with app.app_context():
        ...
        if not User.query.filter_by(email='admin@corps.gov.ng').first():
            admin = User(
                state_code='ADMIN001',
        ...
        if not CDSchedule.query.filter_by(schedule_date=date.today()).first():
            locations = Location.query.limit(2).all()  # Schedule first 2 locations for today
